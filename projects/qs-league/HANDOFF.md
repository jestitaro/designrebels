...
- **Importante — Claude Code (esta sesión) no tiene acceso a la base de datos real**: solo edita el código de este repo. No puede leer, borrar ni escribir en Firestore del proyecto `dino-cup` — cero credenciales, cero red hacia Firebase. Cualquier operación sobre datos reales (borrar temporada, cargar un informe, corregir un movimiento a mano) la tiene que hacer un admin real desde la consola de Firebase o desde el panel logueado. Si en un chat nuevo piden "borrá los datos" o "cargá este excel", la respuesta correcta es dar los pasos para que lo haga la persona, no intentar hacerlo.
...
- **El botón "Guardar" seguía bloqueando aunque estuviera habilitado**: quedaba `disabled` hasta elegir persona (bien), pero al hacer click igual exigía meteorito y motivo completos y mostraba un error si faltaban — contradiciendo su propio estado habilitado. Ahora, con persona elegida, "Guardar" simplemente guarda (meteorito/motivo se pueden completar después con "Editar"); la validación de completitud real sigue viviendo solo en "Confirmar y aplicar" / "Siguiente" del wizard.
- **Grid de la fila con el track del botón desactualizado (38px) mientras el botón ya medía 46px**: quedaba desalineado/apretado. Se actualizó `grid-template-columns` de `.absence-row` y `.absence-row--with-date` a 46px, y se le agregó `margin-top` a "Agregar otra ausencia" (quedaba pegada a la última fila, sin aire).
- **(Grave) Borrar `dinocup_matches`/`dinocup_movements` no dejaba nada en cero — volvían solos**: `activateAdminSession()` llamaba a `ensureSeasonSeeded()` en **cada login**, que revisaba si existía un match en la fecha `2026-07-02` y, si no, recreaba desde cero dos matches hardcodeados ("Season 02", 02/07 y 16/07) más un meteorito manual — pisando cualquier borrado manual hecho directo en Firestore. Encima el seed tenía mal el mapeo de apodo para el 3er puesto del 16/07 (`'Heror' → pablo`, cuando el informe real de Kahoot dice que "Heror" es Agustín), o sea que además de resucitar datos viejos, resucitaba el resultado incorrecto. Se eliminó `SEED_MATCHES`, `SEED_MANUAL_PENALTIES`, `ensureSeasonSeeded()` y su llamada — ya no había necesidad de seguir re-sembrando esos datos históricos ahora que hay cargas reales. Si algún día hace falta re-sembrar algo por única vez, hacerlo a mano desde el panel (o un script aparte), nunca en el camino del login.
